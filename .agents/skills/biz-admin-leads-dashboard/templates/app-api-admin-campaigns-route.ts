...
//   audience.kind  ∈ 'all' | 'paid' | 'pending' | 'last_days'
//   audience.days  ∈ 7 | 30 | 90  (chỉ khi kind='last_days')
//
// Headers: x-admin-pass: <password>
//
// Gửi đồng bộ trong handler — với <300 recipients & throttle 200ms sẽ ≤ 60s
// (Vercel hobby plan timeout). Nếu list lớn hơn nên dùng background job.
...
export async function POST(req: NextRequest) {
  ...
  if (!audience) return NextResponse.json({ error: 'invalid_audience' }, { status: 400 });
  ...
  if (recipients.length === 0) {
    return NextResponse.json({ error: 'empty_audience', message: 'Không có khách hàng phù hợp.' }, { status: 400 });
  }

  // 2. Render once — text hoặc HTML; placeholder {{name}}/{{email}} giữ nguyên,
  //    personalize theo từng khách trong loop bên dưới.

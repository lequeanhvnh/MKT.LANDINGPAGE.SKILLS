// app/admin/page.tsx
//
// 1CRM-inspired admin dashboard cho Supabase lead store.
// Sidebar nav (Tổng quan / Leads / Email marketing) + popup nhập pass.
// Charts qua recharts (npm install recharts), email marketing qua /api/admin/campaigns.
//
...
// Khách mẫu dùng khi preview email (placeholder {{name}}/{{email}}).
...
type DashboardData = {
  ...
  previousPeriod: { leads: number; paid: number; revenue: number; conversionRate: number };
  deltas: {
...
export default function AdminPage() {
  ...
  if (!unlocked) {
    return <PasswordGate onUnlock={pass => { setPassword(pass); setUnlocked(true); }} />;
  }

  return (
    <div style={S.app}>
      <Sidebar tab={tab} onChange={setTab} onLogout={() => { setUnlocked(false); setPassword(''); }} />
      <div style={S.main}>
...
function Sidebar({ tab, onChange, onLogout }: { tab: Tab; onChange: (t: Tab) => void; onLogout: () => void }) {
  ...
  return (
    <aside style={S.sidebar}>
      <div style={S.brand}>
        <div style={S.brandDot}>1</div>
        <div>
          <div style={S.brandName}>CRM Lite</div>
      ...
      <button onClick={onLogout} style={S.logoutBtn}>Thoát</button>
    </aside>
  );
}
...
function DashboardTab({ pass }: { pass: string }) {
  ...
  return (
    <>
      <PageHeader title="Tổng quan kinh doanh" subtitle="Theo dõi leads, doanh thu và tỷ lệ chuyển đổi">
        ...
        <>
          ...
          <div style={S.chartRow}>
            <Card title="Xu hướng theo ngày" style={{ flex: 2, minWidth: 0 }}>
              <div style={{ width: '100%', height: 280 }}>
                <ResponsiveContainer>
                  <LineChart data={data.timeseries.map(b => ({ ...b, dateLabel: shortDate(b.date) }))}>
                    ...
                    <Line yAxisId="left" type="monotone" dataKey="leads" name="Leads" stroke="#2563eb" strokeWidth={2} dot={false} />
                    <Line yAxisId="left" type="monotone" dataKey="paid" name="Paid" stroke="#16a34a" strokeWidth={2} dot={false} />
                    <Line yAxisId="right" type="monotone" dataKey="revenue" name="Doanh thu" stroke="#ea580c" strokeWidth={2} dot={false} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            </Card>
            ...
            <Card title="Trạng thái" style={{ flex: 1, minWidth: 260 }}>
              ...
              <div style={S.totalsBox}>
                <div style={S.totalsRow}>
                  ...
                  <span style={S.totalsValue}>{data.totals.allTimeLeads.toLocaleString('vi-VN')} leads</span>
                </div>
                <div style={S.totalsRow}>
          ...
          <Card title="Thanh toán gần đây">
            {data.recentPayments.length === 0 ? (
              <div style={S.empty}>Chưa có thanh toán trong {data.period.label.toLowerCase()}.</div>
            ) : (
              <table style={S.table}>
                <thead>
                  <tr>
                ...
                <tbody>
                  {data.recentPayments.map(p => (
                    <tr key={p.orderId} style={S.tr}>
                      ...
                      <td style={{ ...S.td, textAlign: 'right', fontWeight: 600 }}>{formatVND(p.amount)}</td>
...
function LeadsTab({ pass }: { pass: string }) {
  ...
  return (
    <>
      <PageHeader title="Danh sách khách hàng" subtitle="Tìm kiếm, lọc theo ngày và xuất CSV">
        <button onClick={exportCSV} disabled={!data || data.leads.length === 0} style={S.btnPrimary}>
          ⬇ Xuất CSV
        </button>
      </PageHeader>
      ...
      {data && (
        <div style={S.kpiGrid}>
          <KpiCard label="Tổng" value={data.stats.totalAll.toLocaleString('vi-VN')} icon="📋" accent="#2563eb" />
          <KpiCard label="Đã thanh toán" value={data.stats.totalPaid.toLocaleString('vi-VN')} icon="✓" accent="#16a34a" />
          <KpiCard label="Chưa thanh toán" value={data.stats.totalPending.toLocaleString('vi-VN')} icon="⏳" accent="#f59e0b" />
          <KpiCard label="Doanh thu" value={formatVNDShort(data.stats.revenue)} hint={formatVND(data.stats.revenue)} icon="💰" accent="#ea580c" />
        </div>
      )}
              ...
              <tbody>
                {data.leads.map(l => (
                  <tr key={l.orderId} style={S.tr}>
                    ...
                    <td style={{ ...S.td, textAlign: 'right', fontWeight: 600 }}>{formatVND(l.amount)}</td>
                    <td style={S.td}>
...
function CampaignsTab({ pass }: { pass: string }) {
  ...
  const [audienceKind, setAudienceKind] = useState<'all' | 'paid' | 'pending' | 'last_days'>('all');
  ...
  async function submit(e: FormEvent) {
    ...
    if (!confirm(`Gửi email "${subject}" tới ${audienceLabel(audienceKind, audienceDays)}?\n\nKhông thể huỷ sau khi bắt đầu gửi.`)) return;
    ...
    try {
      const res = await fetch('/api/admin/campaigns', {
        ...
        body: JSON.stringify({
          ...
          bodyFormat,
          fromName: fromName.trim() || undefined,
          audience: audienceKind === 'last_days'
      ...
      setFlash(`Đã gửi ${json.successCount}/${json.recipientCount} email (${json.failCount} lỗi).`);
      setName(''); setSubject(''); setBody(''); setBodyFormat('text'); setFromName('');
      fetchCampaigns();
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Lỗi không xác định');
    } finally {
  ...
  return (
    <>
      <PageHeader title="Email marketing" subtitle="Soạn và gửi chiến dịch email cho khách hàng" />
        ...
        <Card title="Soạn chiến dịch mới" style={{ flex: 1, minWidth: 320 }}>
          <form onSubmit={submit} style={S.form}>
            <Field label="Tên chiến dịch (nội bộ)">
              <input value={name} onChange={e => setName(e.target.value)} placeholder="Khuyến mãi Tết 2026" style={S.input} />
            </Field>
            <Field label="Tiêu đề email (khách thấy)">
              <input value={subject} onChange={e => setSubject(e.target.value)} placeholder="Quà tặng kèm Tết — anh/chị xem ngay nhé" style={S.input} />
            </Field>
            <Field label="Tên người gửi (tuỳ chọn)">
              <input value={fromName} onChange={e => setFromName(e.target.value)} placeholder="Đội ngũ Acme" style={S.input} />
            </Field>
            <Field label="Đối tượng nhận">
              ...
              {audienceKind === 'last_days' && (
                <select
                  value={audienceDays}
                  onChange={e => setAudienceDays(Number(e.target.value) as 7 | 30 | 90)}
            ...
            >
              <div style={S.segmented}>
                {([
                  ...
                  { v: 'html', label: 'HTML' },
                ] as { v: BodyFormat; label: string }[]).map(o => (
                  <button
                    key={o.v}
                    type="button"
                    onClick={() => setBodyFormat(o.v)}
                    style={{ ...S.segmentBtn, ...(bodyFormat === o.v ? S.segmentBtnActive : {}) }}
                  >
                    {o.label}
                  </button>
                ))}
              ...
              <textarea
                ...
                placeholder={bodyFormat === 'html'
                  ? '<p>Chào {{name}},</p>\n<p>Em vừa cập nhật bonus mới cho khoá học…</p>\n<p><a href="https://...">Xem chi tiết</a></p>'
                  : 'Chào {{name}},\n\nEm là Hoàng — em vừa cập nhật bonus mới cho khoá học...\n\nLink chi tiết: https://...'}
                style={{
                  ...S.textarea,
                  ...(bodyFormat === 'html'
            ...
            <div style={S.btnRow}>
              <button type="button" style={S.btnGhost} onClick={openComposePreview} disabled={!body.trim()}>
                👁 Xem trước
              </button>
              <button type="submit" style={{ ...S.btnPrimary, flex: 1 }} disabled={sending}>
                ...
                <tbody>
                  {campaigns.map(c => (
                    <tr
                      ...
                      onClick={() => setPreviewHtml(personalize(c.bodyHtml, PREVIEW_SAMPLE))}
                    >
                      <td style={S.td}>
                        <div style={{ fontWeight: 600 }}>{c.name}</div>
                        <div style={S.tdSubtle}>{c.subject}</div>
                      </td>
                      <td style={S.tdMuted}>{audienceLabel(c.audience.kind, c.audience.days)}</td>
                      <td style={S.td}>
                        <span style={{ ...S.badge, ...campaignStatusStyle(c.status) }}>
                          {campaignStatusLabel(c.status)}
                        </span>
                      </td>
                      <td style={{ ...S.td, textAlign: 'right', color: '#16a34a', fontWeight: 600 }}>{c.successCount}</td>
                      <td style={{ ...S.td, textAlign: 'right', color: c.failCount > 0 ? '#dc2626' : '#9ca3af' }}>{c.failCount}</td>
                      <td style={S.tdMuted}>{c.sentAt ? formatDateTime(c.sentAt) : '—'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <div style={S.modalOverlay} onClick={() => setPreviewHtml(null)}>
          <div style={S.modalCard} onClick={e => e.stopPropagation()}>
            <div style={S.modalHeader}>
              ...
              <span style={S.modalHint}>Hiển thị với khách mẫu “{PREVIEW_SAMPLE.name}”</span>
              <button type="button" style={S.modalClose} onClick={() => setPreviewHtml(null)}>
                ✕
              </button>
            </div>
            <iframe title="Xem trước email" srcDoc={previewHtml} sandbox="" style={S.previewFrame} />
          </div>
        </div>
      )}
    </>
...
function KpiCard({
  label, value, hint, delta, deltaPoints, icon, accent,
}: {
  label: string;
  value: string;
  hint?: string;
  delta?: number | null;       // pct change
  deltaPoints?: number;        // điểm phần trăm tuyệt đối
  icon?: string;
  accent?: string;
}) {
  return (
    <div style={S.kpiCard}>
      <div style={S.kpiTop}>
        ...
        {icon && (
          <div style={{ ...S.kpiIcon, background: accent ? accent + '15' : '#f3f4f6', color: accent ?? '#374151' }}>
            {icon}
          </div>
        )}
      </div>
...
function DeltaBadge({ pct, points }: { pct?: number; points?: number }) {
  ...
  if (v === 0) return <span style={{ ...S.delta, color: '#6b7280', background: '#f3f4f6' }}>— 0{pct !== undefined ? '%' : 'đ'}</span>;
  const up = v > 0;
  const formatted = pct !== undefined
...
function PeriodPill({ value, onChange }: { value: 7 | 30 | 90; onChange: (v: 7 | 30 | 90) => void }) {
  return (
    <div style={S.pill}>
...
function audienceLabel(kind: string, days?: number): string {
  switch (kind) {
    case 'all': return 'Tất cả khách hàng';
    case 'paid': return 'Đã thanh toán';
    case 'pending': return 'Chưa thanh toán';
    case 'last_days': return `${days ?? '?'} ngày gần nhất`;
    default: return kind;
  }
}

function campaignStatusLabel(s: Campaign['status']): string {
  return s === 'pending' ? 'Chờ gửi'
    : s === 'sending' ? 'Đang gửi'
    : s === 'sent' ? 'Đã gửi'
    : 'Thất bại';
}
...
const S: Record<string, React.CSSProperties> = {
  // Layout
  app: { display: 'flex', minHeight: '100vh', background: BG, fontFamily: FONT, color: TEXT },
  sidebar: {
  ...
  // Sidebar
  brand: { display: 'flex', alignItems: 'center', gap: 10, padding: '4px 8px 16px', borderBottom: '1px solid #1e293b' },
  brandDot: {
    width: 32, height: 32, borderRadius: 8, background: PRIMARY, color: '#fff',
  ...
  navItem: {
    ...
    background: 'transparent', border: 'none', color: '#cbd5e1', borderRadius: 8,
    cursor: 'pointer', textAlign: 'left', fontSize: 13.5, fontWeight: 500,
  },
  ...
  kpiCard: {
    background: CARD_BG, padding: 18, borderRadius: 12, border: `1px solid ${BORDER}`,
  ...
  kpiFooter: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8, marginTop: 'auto' },
  ...
  // Totals box (under donut)
  totalsBox: { borderTop: `1px solid ${BORDER}`, marginTop: 12, paddingTop: 12, display: 'flex', flexDirection: 'column', gap: 6 },
  ...
  pill: { display: 'flex', background: '#fff', borderRadius: 8, padding: 3, border: `1px solid ${BORDER}` },
  pillBtn: {
  ...
  // Buttons
  btnPrimary: {
    padding: '9px 16px', fontSize: 13, fontWeight: 600, color: '#fff', background: PRIMARY,
    border: 'none', borderRadius: 8, cursor: 'pointer',
  },
  btnGhost: {
    padding: '8px 14px', fontSize: 12.5, color: TEXT, background: '#fff',
  ...
  input: {
    padding: '9px 12px', fontSize: 13.5, border: `1px solid ${BORDER}`, borderRadius: 8,
    outline: 'none', fontFamily: FONT, background: '#fff',
  },
  textarea: {
    padding: '10px 12px', fontSize: 13.5, border: `1px solid ${BORDER}`, borderRadius: 8,
    outline: 'none', fontFamily: FONT, background: '#fff', resize: 'vertical',
  },
  select: {
    padding: '9px 12px', fontSize: 13, border: `1px solid ${BORDER}`, borderRadius: 8,
  ...
  searchInput: {
    flex: '1 1 240px', padding: '9px 12px', fontSize: 13, border: `1px solid ${BORDER}`,
    borderRadius: 8, outline: 'none', fontFamily: FONT,
  },
  dateInput: { padding: '8px 10px', fontSize: 12.5, border: `1px solid ${BORDER}`, borderRadius: 8 },
  ...
  th: {
    textAlign: 'left', padding: '10px 12px', fontSize: 11.5, fontWeight: 600,
    color: MUTED, background: '#f9fafb', whiteSpace: 'nowrap',
    borderBottom: `1px solid ${BORDER}`, textTransform: 'uppercase', letterSpacing: 0.3,
  },
  td: { padding: '12px 12px', color: TEXT, whiteSpace: 'nowrap' },
  tdMono: { padding: '12px 12px', whiteSpace: 'nowrap', fontFamily: 'ui-monospace, SFMono-Regular, Menlo, monospace', fontSize: 12, color: TEXT },
  tdMuted: { padding: '12px 12px', color: MUTED, whiteSpace: 'nowrap', fontSize: 12 },
  tdSubtle: { fontSize: 11.5, color: SUBTLE, marginTop: 2 },

  badge: { display: 'inline-block', padding: '3px 10px', fontSize: 11, fontWeight: 700, borderRadius: 12, whiteSpace: 'nowrap' },
  ...
  previewFrame: { width: '100%', height: '70vh', border: 'none', background: '#f5f5f7' },
  ...
  gateCard: {
    width: '100%', maxWidth: 380, background: CARD_BG, borderRadius: 14, padding: '32px 28px',
    boxShadow: '0 4px 24px rgba(0,0,0,0.06)', border: `1px solid ${BORDER}`,
  ...
  gateInput: {
    padding: '12px 14px', fontSize: 14, border: `1px solid ${BORDER}`, borderRadius: 10, outline: 'none',
  },
